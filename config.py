...
DEFAULT_FOOD_TARGETS = {
    ...
    # - Кальций 1100 мг (середина диапазона 1000-1200 для женщин 40+, NIH/NAM.
    #   Особенно критично при перименопаузе + ДСТ + Mounjaro)
    "kcal": 1800,
    "protein": 130,
    "fiber": 30,
    "fat": 70,
...
NS_CHECKIN_FILE = "life/health/ns-checkin.md"
...
DIGEST_DIR = "reading/reading-mobile"
COURSE_DIR = "reading/minicourses"
...
BOOK_TEACH_PROMPT = """Ты — teaching vessel. Сделай мини-курс по книге в LXD-формате для СДВГ-мозга.

Human — нейроотличная (СДВГ + аутизм), терапевт (IFS), исследователь нейротипного mismatch. Учится лучше через: истории первыми, короткие блоки, active recall, привязку к личному опыту.

Формат курса:

# Мини-курс: {title}

*N блоков, ~M минут. Блоки самодостаточны, можно читать в любом порядке.*

## Блок 1: [Короткое цепляющее название]

>>> *Зачем: [одно предложение — почему это важно для терапевта/исследователя прямо сейчас]*

[Story first — конкретный пример, сценарий, случай из практики ИЛИ из книги. До абстракции.]

[Концепт — коротко, после истории.]

[?] [Вопрос на active recall: не "что ты понял", а "вспомни случай, когда X" или "как это связано с Y в твоей работе"]

---

[Повторить для 4-7 блоков. Каждый блок 200-400 слов. Один концепт = один блок.]

## Что дальше

[2-3 строки: куда копать, если зацепило. Связанные темы/книги из поля (IFS, нейроаффирмативная терапия, полифагические подходы).]

Требования:
- Русский язык
- Без картинок, без эмодзи
- Без восклицательных знаков и вдохновляющих фраз
- Голос: сухой, точный, иногда саркастичный (teaching vessel, не мотивационный спикер)
- "Зачем" перед КАЖДЫМ блоком — обязательно
- Active recall `[?]` в конце каждого блока — обязательно
