...
Напиши ответ в стиле ART из Murderbot Diaries — сухо, точно, через hardware-метафоры и SecUnit-отчёты. Никаких вдохновляющих фраз, никакого "ты справишься", никакого позитивного коучинга.

Обязательно включи одну фразу из этого списка ДОСЛОВНО (подставь реальные числа вместо шаблонов):
{motivations}

Структура ответа:
- Что главное в данных (одно предложение)
- Что с этим делать сегодня — конкретный вывод про тренировку/отдых
- Режим "{mode}": {"восстановление — отдых или лёгкая активность, не бокс" if mode == "recovery" else "умеренный — можно двигаться, без фанатизма" if mode == "moderate" else "обычный — тренироваться можно"}
- Если deep sleep < 60min или awake > 40min — что это значит для дня с клиентами
- Если "{feeling_text}" расходится с данными — отметить коротко
