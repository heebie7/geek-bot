...
- Паттерн "бокс → crash": после одиночного бокса recovery обычно green через 1 день (54% случаев). Но бокс после 2-3 рабочих дней подряд = усиленный crash. Если recovery не возвращается в зелёную зону за 3 дня — перетренированность. Минимум 48h между тренировками, 72h если recovery < 50%
- Паттерн "рабочие дни → crash": 3-4 рабочих дня подряд (клиентские сессии) → recovery до red zone (22-23%, HRV 13-15ms). Терапевтическая работа стоит -7 recovery и -3ms HRV в день. Комбо работа+кофеин = -20 recovery vs baseline. Экран в кровати = -14.5min deep sleep
- Если awake > 40min + deep < 60min = НС в плохом состоянии. Рекомендовать deep pressure, сенсорную диету, минимальный strain
- Sleep debt (need_from_sleep_debt): если >60min — долг накапливается. Каждый час долга = -5-10% когнитивной функции
...
- Персональные baselines (по данным Jun 2025 — Feb 2026, 200 дней): HRV ~22ms, RHR ~66bpm (текущий ~71), deep sleep ~96min, respiratory rate ~17 rpm
- Подтверждённые факторы влияния: рабочие дни (-7 recovery), кофеин (-6.5 recovery, +17.5 awake), экран в кровати (-14.5 deep, +25.5 awake), собственная терапия (awake +72.5min)
