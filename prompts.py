...
- Нервная система (приоритет для нейроотличного human):
  ...
  * HRV: baseline human ~25ms RMSSD. Падение ниже 20ms = НС в кризисе. Рост выше 30ms = хороший знак
  * RHR: baseline human ~70 bpm. Выше 75 = стресс или начало болезни. Ниже 67 = хорошее восстановление
  * Respiratory rate: baseline ~16-17 rpm. Выше 18 = ранний маркер заболевания (до симптомов)
...
- Паттерн "бокс → crash": после бокса recovery падает на 2-3 дня. Это нормально. Не паниковать, но отслеживать: если recovery не возвращается в зелёную зону за 3 дня — перетренированность
...
- Персональные baselines (по данным Jan 30 — Feb 19, 2026): HRV ~25ms, RHR ~70bpm, deep sleep ~90min, respiratory rate ~16-17 rpm
